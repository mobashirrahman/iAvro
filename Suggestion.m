...
- (id)init {
  ...
  if (self) {
    ...
    // Load emoji data
    NSString *emojiPath = [[NSBundle mainBundle] pathForResource:@"emoji"
                                                          ofType:@"json"];
    if (emojiPath) {
      NSData *emojiData = [NSData dataWithContentsOfFile:emojiPath];
      if (emojiData) {
        NSError *error = nil;
        NSDictionary *emojiDict =
            [NSJSONSerialization JSONObjectWithData:emojiData
                                            options:0
                                              error:&error];
        if (emojiDict && !error) {
          _emojiEmoticons = [[emojiDict objectForKey:@"emoticons"] retain];
          _emojiNamesEn = [[emojiDict objectForKey:@"names_en"] retain];
          _emojiNamesBn = [[emojiDict objectForKey:@"names_bn"] retain];
        }
      }
    }
  }
...
- (void)dealloc {
  ...
  [_emojiEmoticons release];
  [_emojiNamesEn release];
  [_emojiNamesBn release];
  [super dealloc];
}
...
- (NSMutableArray *)getList:(NSString *)term {
      ...
      if (suffix) {
        ...
        if (cached) {
          for (NSString *item in cached) {
            ...
            NSString *word;
            // Again saving humanity cause I'm Superman, no I'm not drunk or on
            // weed :D
            NSInteger cutPos = [item length] - 1;

            NSString *itemRMC =
                [item substringFromIndex:cutPos]; // RMC is Right Most Character
            NSString *suffixLMC =
  ...
  // Emoji suggestions
  NSArray *emojis = [self emojisForTerm:term withSuggestions:_suggestions];
  if (emojis && [emojis count] > 0) {
    [_suggestions addObjectsFromArray:emojis];
  }
...
- (NSArray *)emojisForTerm:(NSString *)term
  ...
  // 1. Emoticon match (e.g., ":)" → "😀")
  if (_emojiEmoticons) {
    NSString *emoticon = [_emojiEmoticons objectForKey:term];
    if (emoticon && ![results containsObject:emoticon]) {
      [results addObject:emoticon];
    }
  }

  // 2. English name match (e.g., "heart" → ["❤️", "💖", ...])
  if (_emojiNamesEn) {
    NSString *lowerTerm = [term lowercaseString];
    NSArray *enEmojis = [_emojiNamesEn objectForKey:lowerTerm];
    if (enEmojis) {
      for (NSString *emoji in enEmojis) {
        if (![results containsObject:emoji]) {
  ...
  // 3. Bengali name match (check each suggestion against Bengali emoji names)
  if (_emojiNamesBn && bnSuggestions) {
    for (NSString *suggestion in bnSuggestions) {
      NSArray *bnEmojis = [_emojiNamesBn objectForKey:suggestion];
      if (bnEmojis) {
        for (NSString *emoji in bnEmojis) {
          if (![results containsObject:emoji]) {
            [results addObject:emoji];
          }
        }
